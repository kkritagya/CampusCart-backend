...
export async function suggestListing(
  ...
  if (!apiKey) {
    throw new HttpException(503, "AI listing assistance is not configured");
  }
        ...
        contents: [
          {
            ...
            parts: [
              {
                text:
                  `Analyze this second-hand marketplace product photo. Write the title and description in ${languageName}. ` +
  ...
  if (!response.ok) {
    ...
    throw new HttpException(502, "AI listing suggestion failed");
  }
  ...
  if (!text) throw new HttpException(502, "Gemini returned no listing suggestion");

  const parsed = JSON.parse(text) as ListingSuggestion;
  return parsed;
}
